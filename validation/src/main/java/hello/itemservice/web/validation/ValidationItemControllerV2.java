...
public class ValidationItemControllerV2 {
    ...
    @InitBinder // 해당 컨트롤러에만 영향줌
    public void init(WebDataBinder dataBinder) {
        dataBinder.addValidators(itemValidator);    // 검증기 추가 -> 해당 컨트롤러에서 검증기 자동으로 적용할 수 있음
    }
...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item, BindingResult bindingResult, RedirectAttributes redirectAttributes) {
        /**
         * 주의 사항 : BindingResult 파라미터 위치는 ModelAttribute 객체 다음에 와야한다
         */
        // 검증 오류 결과를 보관 (어떤 필드에서 오류가 발생했는지 구분하기 위해 "필드명"을 "key"로 사용)
...
//            errors.put("quantity", "수량은 최대 9,999 까지 허용합니다.");
        ...
        // 특정 필드가 아닌 복합 룰 검증 - (key : globalError)
...
//                errors.put("globalError", "가격 * 수량의 합은 10,000원 이상이어야 합니다. 현재 값 = " + resultPrice);
        ...
        // 검증에 실패하면 다시 입력 폼으로
//        if (!errors.isEmpty()) {
        if(bindingResult.hasErrors()) { // bingResult는 자동으로 뷰로 넘어감
//            model.addAttribute("errors", errors);
            return "validation/v2/addForm"; // redirect X
        }
